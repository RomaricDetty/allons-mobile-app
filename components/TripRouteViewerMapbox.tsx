 ...
 */
export default function TripRouteViewerMapbox({ booking }: TripRouteViewerMapboxProps) {
    ...
    const addressUpdateTimeoutRef = useRef<ReturnType<typeof setTimeout> | null>(null);
     ...
     */
    const toMapboxCoordinates = useCallback((coord: { latitude: number; longitude: number }): [number, number] => {
    ...
    const centerMapOnRoute = useCallback(() => {
        ...
        const zoom = maxDelta > 0 ? Math.max(8, Math.min(15, 15 - Math.log2(maxDelta * 100))) : 13;
     ...
     */
    const handleMapPress = useCallback(async (coordinate: [number, number]) => {
    ...
    const startCityCode = useMemo(() => getCityCode(booking.trip?.stationFrom?.city || ''), [booking.trip?.stationFrom?.city, getCityCode]);
    const endCityCode = useMemo(() => getCityCode(booking.trip?.stationTo?.city || ''), [booking.trip?.stationTo?.city, getCityCode]);
    ...
    return (
        <View style={[styles.container, { backgroundColor }]}>
            ...
            <MapView
                ref={mapRef}
                style={styles.map}
                styleURL={isDarkMode ? 'mapbox://styles/mapbox/dark-v11' : 'mapbox://styles/mapbox/standard-v11'}
                onPress={(feature: any) => {
                    if (!isManualMode) return;
                    ...
                    if (feature.geometry && 'coordinates' in feature.geometry) {
                        ...
                        if (typeof feature.coordinates.longitude === 'number' && typeof feature.coordinates.latitude === 'number') {
                            coords = [feature.coordinates.longitude, feature.coordinates.latitude];
                        }
                    }
            ...
            >
                ...
                {endPoint && (
                    <PointAnnotation
                        id="end-point"
                        coordinate={toMapboxCoordinates(endPoint)}
                ...
                {passengerLocation && (
                    <PointAnnotation
                        ...
                        coordinate={toMapboxCoordinates({
                            latitude: passengerLocation.latitude,
                            longitude: passengerLocation.longitude,
                        })}
                        title="Vous êtes ici"
