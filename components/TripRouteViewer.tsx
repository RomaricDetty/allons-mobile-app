...
export default function TripRouteViewer({ booking }: TripRouteViewerProps) {
  ...
  const addressUpdateTimeoutRef = useRef<ReturnType<typeof setTimeout> | null>(
    null,
  ); // Debounce pour les mises à jour d'adresse
  ...
  const initialRegion = useMemo(() => {
    const defaultLat =
      startPoint?.latitude ?? passengerLocation?.latitude ?? 5.33542;
    const defaultLon =
      startPoint?.longitude ?? passengerLocation?.longitude ?? -4.00351;
    return {
      latitude: defaultLat,
  ...
  const handleMapPressCallback = useCallback(
    (event: any) => {
